...
interface SceneEnvProps {
  /** World radius that must stay visible (stacked orbits grow past the default fog). */
  extent?: number
}
...
export function SceneEnv({ extent = 18 }: SceneEnvProps = {}) {
  ...
  const fogFar = Math.max(tron ? 26 : 32, extent * 2.1)
  ...
  return (
    <>
      ...
      />
      <ShowroomBackdrop radius={Math.max(22, extent * 1.45)} />
